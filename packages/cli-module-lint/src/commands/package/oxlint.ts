...
export default async (opts: PackageLintOptions): Promise<boolean> => {
  ...
  // Map ESLint-specific formatter to oxlint equivalent; other values
  // are passed through and oxlint will validate them itself.
  const rawFormat = opts.format ?? 'default';
  const format =
    rawFormat === 'eslint-formatter-friendly' ? 'default' : rawFormat;
  ...
  const configJson = loadAndFinalizeConfig(
    role,
    targetPaths.dir,
    targetPaths.rootDir,
  );
