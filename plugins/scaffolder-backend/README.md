...
- **`task-step-execution`**: Tracks`initiation` and `completion` of a scaffolder task step execution
- **`task-step-skip`**: Tracks steps skipped due to`if` conditionals not being met
- **`task-step-iteration`**: Tracks the step execution of each iteration of a task step that contains the`each` field.
